...
echo "식당 아이디:".$row["id"]." 카테고리:".$row["categori"]."<br>";
...
?>
			...
			.main_header{
				...
				<style>
				.overlaybox {font-family: 'helvetica'}
				.overlaybox {
					position:relative;width:150px;height:180px;border-radius: 5%; position: absolute; top: -215px; left: -70px;background-color: rgb(61, 63, 74);
					padding:15px 10px;
				}
				.overlaybox div, ul {margin:0;padding:0;}
				.overlaybox li {list-style: none;}
				.overlaybox .boxtitle {color:#fff;font-size:16px;font-weight:bold; text-align: right; margin-top: -8px; margin-right: 3px;}
				.categori {
					position: relative; width: 127px; height: 20px; text-align: right; font-size: 12px; color: #fff;
				}
				.overlaybox .text {color:#fff;font-weight:bold; font-size: 12px; position: absolute; top: 30px; left: 10px;}
				.first .triangle {
				...
				.overlaybox ul {width:127px;}
				.overlaybox li {position:relative;margin-bottom:6px;background:#2b2d36;padding:5px 8px;color:#aaabaf;line-height: 1; border-radius: 5%;}
				.overlaybox li span {display:inline-block;}
				.overlaybox li .addr {font-size:12px;font-weight:bold;}
				.overlaybox li .addr1 {
					font-size:10px; white-space: normal;
				}
				.overlaybox li .addr2 {font-size:10px;}
				.overlaybox .review {width:127px;height: 70px; overflow: hidden; word-break: break-all;}
				.overlaybox .review:hover {overflow: visible;}
				.overlaybox li .review1 {font-size:12px;font-weight:bold;}
				.overlaybox li .review2 {
					font-size:10px; white-space: normal;
				}
				.overlaybox li:hover {color:#fff;background:#d24545; transform: scale(1.1);}
				</style>
				...
				<ul>
					...
					if(!$s_id){?>
						...
						<?php } else { ?>
							<p id='hello'><?php echo $s_name; ?>님, 안녕하세요.
							...
							<?php if($s_id == "admin"){ ?>
								...
								<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=26fdf226a690f77f33e7a8f67ee40ac1"></script>
								<script>
								...
								var positions = [
									{
										index: 1,
			title: '올래곱창', 
			latlng: new kakao.maps.LatLng(37.501108, 127.053060),
